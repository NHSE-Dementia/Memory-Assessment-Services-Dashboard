...
SELECT
	*
	,ROW_NUMBER() OVER(PARTITION BY [UniqServReqID],[Der_Person_ID] ORDER BY [CodedDiagTimestamp] ASC) AS RowIDEarliest
	,ROW_NUMBER() OVER(PARTITION BY [UniqServReqID],[Der_Person_ID] ORDER BY [CodedDiagTimestamp] DESC) AS RowIDLatest
