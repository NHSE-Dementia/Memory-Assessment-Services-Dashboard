...
--The next table ([NHSE_Sandbox_MentalHealth].[dbo].[TEMP_DEM_MAS_DIAG_Ranking]) ranks the diagnosis dates for use later in the script
 IF OBJECT_ID ('[MHDInternal].[TEMP_DEM_MAS_DIAG]') IS NOT NULL DROP TABLE [MHDInternal].[TEMP_DEM_MAS_DIAG]
...
SELECT DISTINCT
	...
	,Der_Person_ID
	,UniqServReqID
	,CodedDiagTimestamp AT TIME ZONE 'GMT Standard Time' AS CodedDiagTimestamp
...
SELECT DISTINCT 
	...
	ELSE 'Dementia' END AS 'Diagnosis Area'
FROM [MESH_MHSDS].[MHS605SecDiag]  r
WHERE 
(r.[SecDiag] IN 
...
IF OBJECT_ID ('[MHDInternal].[TEMP_DEM_MAS_Contact]') IS NOT NULL DROP TABLE [MHDInternal].[TEMP_DEM_MAS_Contact]
SELECT
	UniqServReqID
	,Der_Person_ID as Person_ID
	,MIN(CareContDate) AS FirstContactDate
INTO [MHDInternal].[TEMP_DEM_MAS_Contact]
FROM [MESH_MHSDS].[MHS201CareContact] 
